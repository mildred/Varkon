...
static bool   autofit;                       /* Fit media TRUE/FALSE */

/*
***Prototypes for internal functions.
*/
static void start_print(WPGWIN *gwinpt,bool portrait);
 ...
 ******************************************************!*/
  ...
  {
   char     title[81],port[81],land[81],reject[81],conf[81],
            help[81],porttt[81],landtt[81],rejecttt[81],
            conftt[81],helptt[81];
   char    *typ[20];
   int      l1,l2,ly,lm,bh,main_dx,main_dy,wm_x1,wm_y1,
            wm_x2,wm_y2,alt_x,alt_y,butlen1,butlen2;;
   unsigned int dum1,dum2;
   DBint    iwin_id,land_id,port_id,reject_id,conf_id,help_id,
            but_id;
   WPGWIN  *gwinpt;
   WPWIN   *winptr;
   WPIWIN  *iwinpt;
...
*/
   butlen1 = 0;
   if ( WPstrl(port)   > butlen1 ) butlen1 = WPstrl(port);
   if ( WPstrl(land)   > butlen1 ) butlen1 = WPstrl(land);
   butlen1 *= 1.8;
...
/*
***Portrait and landscape buttons.
*/
   alt_x  = ly;
   alt_y  = ly;
...
*/
   if ( but_id == port_id )
     {
     start_print(gwinpt,TRUE);
     goto exit;
     }
/*
***Landscape button. Don't permit landscape if printer is manually configured.
*/
   else if ( but_id == land_id )
     {
     if ( autofit )
       {
       start_print(gwinpt,FALSE);
       goto exit;
       }
     else
       {
 ...
 *          portrait = TRUE if portrait, landscape otherwise.
 *
 *      (C)2007-04-08 J.Kjellander
 *
 ******************************************************!*/
...
*/
   ...
   fclose(fp);
/*
***Scale and orientation for autofit.
*/
   if ( autofit )
     {
     if ( portrait )
       {
       sx = media_x/(gwinpt->vy.modwin.xmax - gwinpt->vy.modwin.xmin);
       sy = media_y/(gwinpt->vy.modwin.ymax - gwinpt->vy.modwin.ymin);
       if ( sx < sy ) s = sx;
       else           s = sy;
       sprintf(params," -s%g ",s);
       }
     else
       {
       sx = media_y/(gwinpt->vy.modwin.xmax - gwinpt->vy.modwin.xmin);
       sy = media_x/(gwinpt->vy.modwin.ymax - gwinpt->vy.modwin.ymin);
       if ( sx < sy ) s = sx;
       else           s = sy;
       sprintf(params," -s%g -v90 -x-%g",s,media_x);
       }
     }
/*
...
*/
   if ( fgets(line,V3STRLEN,fp) == NULL ) goto error;
   if ( strncmp(line,"Autofit:",8) != 0 ) goto error;
   if ( strncmp(&line[9],"True",4) == 0 ) autofit = TRUE;
   else                                   autofit = FALSE;
/*
...
***Write Autofit:
*/
   if ( autofit ) fprintf(fp,"Autofit: True\n");
   else           fprintf(fp,"Autofit: False\n");
/*
***Close the plotter configuration file.
*/
   fclose(fp);
...
/*      The configuration dialogue for printing.
 *
 *      (C)2007-04-08 J.Kjellander
 *
 ******************************************************!*/

  {
   char     title[81],drv[81],drvtt[81],cmd[81],cmdtt[81],
            ...
            mystr[81],pscl[81],autos[81],autostt[81],man[81],
            mantt[81],okey[81],reject[81],help[81],okeytt[81],
            rejecttt[81],helptt[81];
   int      ly,bh,lm,main_x,main_y,main_dx,main_dy,butlen1,
            butlen2,butlen3,edlen,l1,l2,alt_x,alt_y;
   DBint    iwin_id,drv_id,cmd_id,mx_id,my_id,autos_id,man_id,
...
*/
   ...
   if ( !WPgrst("varkon.print.mediasize",media) )          strcpy(media,"Mediasize");
   ...
   if ( !WPgrst("varkon.print.auto_manual",pscl) )         strcpy(pscl,"Scale");
...
*/
   ...
   alt_y += lm;
   WPmcbu((wpw_id)iwin_id,alt_x,alt_y,main_dx,bh,(short)0,
                        pscl,pscl,"",WP_BGND1,WP_FGND,&but_id);

   alt_x  = 3*ly;
   alt_y += bh + lm;
   if ( autofit )
     WPmcbu((wpw_id)iwin_id,alt_x,alt_y,butlen2,bh,(short)1,
                           autos,autos,"",WP_BGND3,WP_FGND,&autos_id);
   else
     WPmcbu((wpw_id)iwin_id,alt_x,alt_y,butlen2,bh,(short)1,
                           autos,autos,"",WP_BGND2,WP_FGND,&autos_id);

   butptr = (WPBUTT *)iwinpt->wintab[autos_id].ptr;
   ...
   if ( autofit )
     ...
     WPmcbu((wpw_id)iwin_id,alt_x,alt_y,butlen2,bh,(short)1,
                             man,man,"",WP_BGND3,WP_FGND,&man_id);
